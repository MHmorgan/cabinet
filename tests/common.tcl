# Missing features:
#   * csv - https://github.com/tcltk/tcllib/tree/master/modules/csv
#   * data structures - https://github.com/tcltk/tcllib/tree/master/modules/struct
#   * fileutil - https://github.com/tcltk/tcllib/tree/master/modules/fileutil
#   * httpwget - https://github.com/tcltk/tcllib/tree/master/modules/httpwget
#   * json - https://github.com/tcltk/tcllib/tree/master/modules/json
#   * markdown - https://github.com/tcltk/tcllib/tree/master/modules/markdown
#   * textutil - https://github.com/tcltk/tcllib/tree/master/modules/textutil
#   * yaml - https://github.com/tcltk/tcllib/blob/master/modules/yaml/yaml.tcl

namespace eval ::common {
  namespace export log warn err bail setvar setfile setdir header
  namespace export input confirm
}
...
# Arguments:
#   MSG -> Log message
#   END -> End of line, default is \n
#
proc ::common::log {msg {end \n}} {
  puts -nonewline stdout "\[*\] $msg$end"
  flush stdout
}
...
#   END -> End of line, default is \n
#
proc ::common::warn {msg {end \n}} {
  puts -nonewline stderr "\[!\] $msg$end"
  flush stderr
}
...
# bail MSG ?END?
#
#   Print an error message with [!!] prefix and exit with code 1
#
# Arguments:
#   MSG -> Error message
#   END -> End of line, default is \n
#
proc ::common::bail {msg {end \n}} {
  err $msg $end
  exit 1
}

proc ::common::_setvar {name val env opt lvl isdir isfile} {
  upvar $lvl $name var

  if {[string is true $env]} {
    if {[info exists ::env($name)]} {
      set var $::env($name)
    } elseif {$opt} {
      set var $val
    } else {
      throw {MH NO_ENV} "Environment variable not found: $name"
    }
  } elseif {![string is false $env]} {
    if {[info exists ::env($env)]} {
      set var $::env($env)
    } elseif {$opt} {
      set var $val
    } else {
      throw {MH NO_ENV} "Environment variable not found: $env"
    }
  } else {
    set var $val
  }

  if {$isdir && ![file isdirectory $var]} {
    throw {MH NO_DIR} "Not a directory: $var"
  }
  if {$isfile && ![file isfile $var]} {
    throw {MH NO_FILE} "Not a file: $var"
  }
}

# setvar NAME ?VAL? ?ENV? ?OPT?
#
#   Set a variable, optionally reading the variable from the environment.
#   If the environment variable isn't found {MH NO_ENV} error is thrown.
#
# Arguments:
#   NAME  Name of the variable
#   VAL   Value of the variable. Default is {}
#   ENV   If true, fetch the value from the environment. If the argument
#         is not boolean the value is assumed to be the name of the
#         environment variable. Default is false.
#   OPT   Environment variable is optional if true, in which case `val`
#         is used as the default. Default is true.
#
proc ::common::setvar {name {val {}} {env 0} {opt 1}} {
  ::common::_setvar $name $val $env $opt 2 false false
}

# setfile NAME PATH ?ENV? ?OPT?
#
#   Set a file variable, optionally reading the variable from the environment.
#   If the environment variable isn't found {MH NO_ENV} error is thrown.
#   If the path isn't a file {MH NO_FILE} error is thrown.
#
# Arguments:
#   NAME  Name of the variable
...
#         is used as the default. Default is true.
#
proc ::common::setfile {name path {env 0} {opt 1}} {
  ::common::_setvar $name $path $env $opt 2 false true
}

# setdir NAME PATH ?ENV? ?OPT?
#
#   Set a directory variable, optionally reading the variable from the environment.
#   If the environment variable isn't found {MH NO_ENV} error is thrown.
#   If the path isn't a directory {MH NO_DIR} error is thrown.
#
# Arguments:
#   NAME  Name of the variable
#   PATH  Path of the variable
...
#         is used as the default. Default is true.
#
proc ::common::setdir {name path {env 0} {opt 1}} {
  ::common::_setvar $name $path $env $opt 2 true false
}

# input PROMPT
#
#   Read input from stdin, returning the input text.
#
# Arguments:
#   PROMPT  Prompt text
#
proc ::common::input {prompt} {
...
# confirm PROMPT
#
#   Make the user confirm something, returning 1 or 0
#
# Arguments:
#   PROMPT  Prompt text. A (Y/n) postfix will be appended
#
proc ::common::confirm {prompt} {
  set txt [::common::input "$prompt (Y/n) "]
  if {[string length $txt]==0 || [regexp {^\s*[Yy](es?)?\s*$} $txt]} {
    return 1
  } else {
    return 0
  }
...
# header TEXT
#
#   Print a single-line header message. A border will be printed
#   under the header text.
#
# Arguments:
#   TEXT  Header text
#
proc ::common::header {text} {
  puts ""
  puts $text
  puts [string repeat = [string length $text]]
  puts ""
}
